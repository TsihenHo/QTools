...
if(false){
        ...
        print("toast(String text) 或 Toast(String text)");
        "发送 Toast。";

        print("createData(String isGroup, String uin)");
        "手动创建一个 MessageData。isGroup 是否群聊，uin 为QQ号码";

        print("createPrivateChatData(String group, String person)");
        "手动从群聊创建一个临时会话的 MessageData。group 是群号码，person 是QQ号码";

        print("send(MessageData data, String text)");
        "根据 data 发送内容为 text 的消息";

        print("send(MessageData data, String text, String[] atList)");
        "根据 data 发送内容为 text 的消息并艾特 atlist 中的成员。atList 中存放被艾特的群员的QQ号";

        print("record(MessageData data)");
        "根据 data 撤回消息";

        print("sendCard(MessageData data, String source)");
        "根据 data 发送卡片消息。source: 源代码";

        print("sendPhoto(MessageData data, String path)");
        "根据 data 发送图片消息。path: 路径";

        print("sendTip(MessageData data, String text)");
        "根据 data 发送灰色提示消息，样式和“XXX撤回了一条消息”一样。text: 内容";

        print("sendShowPhoto(MessageData data, String path, int type)");
        "根据 data 发送秀图，type: 0-秀图 1-幻影 2-抖动 3-生日 4-爱你 5-征友";

        print("sendShake(MessageData data)");
        "根据 data 发送窗口抖动";

        print("sendPtt(MessageData data, String path, String title, String showTime)");
